...
async fn test_common_phrase_crud() {
    ...
    let (status, body) = app
        .get_with_auth(
            "/api/v1/templates/common-phrases?category=diagnosis",
            &token,
        )
        .await;
    assert_eq!(status, StatusCode::OK);
    ...
    assert!(phrases
        .iter()
        .all(|p| p["category"].as_str().unwrap() == "diagnosis"));

    // 4. 获取单个常用语
    let (status, body) = app
        .get_with_auth(
            &format!("/api/v1/templates/common-phrases/{}", phrase_id),
            &token,
        )
        .await;
    assert_eq!(status, StatusCode::OK);
    ...
    assert_eq!(
        body["data"]["content"].as_str().unwrap(),
        "风寒感冒，症见恶寒重、发热轻、无汗、头痛"
    );

    // 6. 使用常用语（增加使用次数）
    let (status, _) = app
        .post_with_auth(
            &format!("/api/v1/templates/common-phrases/{}/use", phrase_id),
            json!({}),
            &token,
        )
        .await;
...
async fn test_prescription_template_crud() {
    ...
    let (status, body) = app
        .get_with_auth(
            "/api/v1/templates/prescription-templates?search=感冒",
            &token,
        )
        .await;
    assert_eq!(status, StatusCode::OK);
    ...
    assert!(templates
        .iter()
        .any(|t| t["name"].as_str().unwrap().contains("感冒")
            || t["diagnosis"].as_str().unwrap().contains("感冒")));

    // 4. 获取单个处方模板
    let (status, body) = app
        .get_with_auth(
            &format!("/api/v1/templates/prescription-templates/{}", template_id),
            &token,
        )
        .await;
    assert_eq!(status, StatusCode::OK);
    ...
    let (status, _) = app
        .post_with_auth(
            &format!(
                "/api/v1/templates/prescription-templates/{}/use",
                template_id
            ),
            json!({}),
            &token,
...
async fn test_template_search_and_filter() {
    ...
    for (category, content) in phrases {
        ...
        app.post_with_auth("/api/v1/templates/common-phrases", dto, &token)
            .await;
    }
    ...
    assert!(results
        .iter()
        .all(|p| p["content"].as_str().unwrap().contains("感冒")));
    ...
    assert!(results
        .iter()
        .all(|p| p["category"].as_str().unwrap() == "advice"));

    // 测试分页
    let (status, body) = app
        .get_with_auth(
            "/api/v1/templates/common-phrases?page=1&page_size=2",
            &token,
        )
        .await;
    assert_eq!(status, StatusCode::OK);
