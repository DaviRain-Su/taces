...
async fn test_create_circle() {
    ...
    assert_eq!(body["message"].as_str().unwrap(), "Circle created successfully");
    
    let circle = &body["data"];
    assert_eq!(circle["name"].as_str().unwrap(), "Test Circle");
    assert_eq!(circle["description"].as_str().unwrap(), "A test circle for unit testing");
...
async fn test_get_circles() {
    ...
    app.post_with_auth("/api/v1/circles", create_dto1, &token1).await;
    ...
    app.post_with_auth("/api/v1/circles", create_dto2, &token1).await;
    ...
    // Search by category
    let (status, body) = app.get_with_auth("/api/v1/circles?category=健康", &token2).await;
    assert_eq!(status, StatusCode::OK);
    
    let circles = body["data"]["circles"].as_array().unwrap();
    assert!(circles.iter().all(|c| c["category"].as_str().unwrap() == "健康"));
}
...
async fn test_join_and_leave_circle() {
    ...
    let (status, body) = app
        .post_with_auth(&format!("/api/v1/circles/{}/join", circle_id), json!({}), &token2)
        .await;
    assert_eq!(status, StatusCode::OK);
    assert_eq!(body["message"].as_str().unwrap(), "Joined circle successfully");
    ...
    let (status, body) = app
        .post_with_auth(&format!("/api/v1/circles/{}/leave", circle_id), json!({}), &token2)
        .await;
    assert_eq!(status, StatusCode::OK);
    assert_eq!(body["message"].as_str().unwrap(), "Left circle successfully");
...
async fn test_circle_member_management() {
    ...
    // Members join
    app.post_with_auth(&format!("/api/v1/circles/{}/join", circle_id), json!({}), &member_token)
        .await;
    app.post_with_auth(&format!("/api/v1/circles/{}/join", circle_id), json!({}), &member3_token)
        .await;
    ...
    // Get members list
    let (status, body) = app
        .get_with_auth(&format!("/api/v1/circles/{}/members", circle_id), &owner_token)
        .await;
    if status != StatusCode::OK {
