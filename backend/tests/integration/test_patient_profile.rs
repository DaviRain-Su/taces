...
async fn test_patient_profile_crud() {
    ...
    let (status, body) = app.post_with_auth("/api/v1/patient-profiles", self_profile, &patient_token).await;
    println!("Create self profile response: status={:?}, body={:?}", status, body);
    ...
    let (status, body) = app.post_with_auth("/api/v1/patient-profiles", family_profile, &patient_token).await;
    assert_eq!(status, StatusCode::OK);
    let family_profile_id = body["data"]["id"].as_str().unwrap();
    assert!(!body["data"]["is_default"].as_bool().unwrap()); // Should not be default

    // List profiles
    let (status, body) = app.get_with_auth("/api/v1/patient-profiles", &patient_token).await;
    assert_eq!(status, StatusCode::OK);
    ...
    // Get default profile
    let (status, body) = app.get_with_auth("/api/v1/patient-profiles/default", &patient_token).await;
    assert_eq!(status, StatusCode::OK);
    ...
    assert_eq!(body["message"].as_str().unwrap(), "Cannot delete self profile");

    // Delete family profile
    let (status, _) = app.delete_with_auth(
        &format!("/api/v1/patient-profiles/{}", family_profile_id),
        &patient_token
    ).await;
    assert_eq!(status, StatusCode::OK);
...
async fn test_patient_profile_permissions() {
    ...
    let (_patient1_id, patient1_account, patient1_password) = create_test_user(&app.pool, "patient").await;
    let (_patient2_id, patient2_account, patient2_password) = create_test_user(&app.pool, "patient").await;
    ...
    let (status, body) = app.post_with_auth("/api/v1/patient-profiles", profile_dto, &patient1_token).await;
    assert_eq!(status, StatusCode::OK);
    ...
    let (status, _) = app.post_with_auth("/api/v1/patient-profiles", profile_dto, &doctor_token).await;
    assert_eq!(status, StatusCode::FORBIDDEN);

    // Patient2 cannot see Patient1's profiles
    let (status, body) = app.get_with_auth("/api/v1/patient-profiles", &patient2_token).await;
    assert_eq!(status, StatusCode::OK);
    assert_eq!(body["data"].as_array().unwrap().len(), 0);

    // Patient2 cannot access Patient1's specific profile
    let (status, _) = app.get_with_auth(
...
async fn test_patient_profile_id_validation() {
    ...
    for invalid_id in invalid_ids {
        ...
        let (status, body) = app.post_with_auth("/api/v1/patient-profiles", profile_dto, &patient_token).await;
        println!("Testing invalid ID {}: status={:?}, body={:?}", invalid_id, status, body);
        assert!(status == StatusCode::BAD_REQUEST, "ID {} should be invalid", invalid_id);
    }
    ...
    let (status, _) = app.post_with_auth("/api/v1/patient-profiles", valid_15_digit, &patient_token).await;
    assert_eq!(status, StatusCode::OK);
}
...
async fn test_patient_profile_duplicate_id() {
    ...
    let (status, _) = app.post_with_auth("/api/v1/patient-profiles", profile_dto.clone(), &patient1_token).await;
    assert_eq!(status, StatusCode::OK);

    // Patient2 tries to create profile with same ID number
    let (status, body) = app.post_with_auth("/api/v1/patient-profiles", profile_dto, &patient2_token).await;
    assert_eq!(status, StatusCode::CONFLICT);
    assert_eq!(body["message"].as_str().unwrap(), "This ID number is already registered");
}
...
async fn test_patient_profile_first_default() {
    ...
    let (status, body) = app.post_with_auth("/api/v1/patient-profiles", family_profile, &patient_token).await;
    assert_eq!(status, StatusCode::OK);
    assert!(body["data"]["is_default"].as_bool().unwrap(), "First profile should be default");
    ...
    let (status, body) = app.post_with_auth("/api/v1/patient-profiles", self_profile, &patient_token).await;
    assert_eq!(status, StatusCode::OK);
    assert!(body["data"]["is_default"].as_bool().unwrap(), "Self profile should be default");
    ...
    for profile in profiles {
        if profile["relationship"] == "family" {
            assert!(!profile["is_default"].as_bool().unwrap(), "Family profile should not be default");
        }
    }
}