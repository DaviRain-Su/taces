...
async fn test_get_content_type() {
    // Images
    assert_eq!(FileStorageService::get_content_type("photo.jpg"), "image/jpeg");
    assert_eq!(FileStorageService::get_content_type("photo.png"), "image/png");
    assert_eq!(FileStorageService::get_content_type("photo.gif"), "image/gif");
    
    // Documents
    assert_eq!(FileStorageService::get_content_type("report.pdf"), "application/pdf");
    assert_eq!(FileStorageService::get_content_type("data.xlsx"), 
        "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
    
    // Videos
    assert_eq!(FileStorageService::get_content_type("video.mp4"), "video/mp4");
    assert_eq!(FileStorageService::get_content_type("video.avi"), "video/x-msvideo");
    
    // Audio
    assert_eq!(FileStorageService::get_content_type("audio.mp3"), "audio/mpeg");
    assert_eq!(FileStorageService::get_content_type("audio.wav"), "audio/wav");
    
    // Unknown
    assert_eq!(FileStorageService::get_content_type("file.xyz"), "application/octet-stream");
}
...
async fn test_presigned_urls() {
    ...
    let download_url = FileStorageService::generate_presigned_download_url(
        &s3_client,
        file_path,
        300,
    )
