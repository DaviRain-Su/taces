...
async fn test_patient_group_crud() {
    ...
    let (status, body) = app.post_with_auth("/api/v1/patient-groups", create_dto, &doctor_token).await;
    println!("Create group response: status={:?}, body={:?}", status, body);
    assert_eq!(status, StatusCode::OK);
    assert!(body["success"].as_bool().unwrap());
    let group_id = body["data"]["id"].as_str().unwrap();

    // List groups
    let (status, body) = app.get_with_auth("/api/v1/patient-groups", &doctor_token).await;
    assert_eq!(status, StatusCode::OK);
    ...
    let (status, _body) = app.delete_with_auth_body(
        &format!("/api/v1/patient-groups/{}/members", group_id),
        remove_members_dto,
        &doctor_token
    ).await;
...
async fn test_patient_group_limit() {
    ...
    for i in 1..=5 {
        ...
        let (status, _) = app.post_with_auth("/api/v1/patient-groups", create_dto, &doctor_token).await;
        assert_eq!(status, StatusCode::OK);
    }
    ...
    assert_eq!(body["message"].as_str().unwrap(), "Maximum 5 patient groups allowed");
}
...
async fn test_patient_group_permissions() {
    ...
    // Create two doctors
    let (doctor1_user_id, doctor1_account, doctor1_password) = create_test_user(&app.pool, "doctor").await;
    ...
    let (doctor2_user_id, doctor2_account, doctor2_password) = create_test_user(&app.pool, "doctor").await;
    ...
    let (status, body) = app.post_with_auth("/api/v1/patient-groups", create_dto, &doctor1_token).await;
    assert_eq!(status, StatusCode::OK);
    let group_id = body["data"]["id"].as_str().unwrap();
    ...
    let (status, _) = app.post_with_auth("/api/v1/patient-groups", create_dto, &patient_token).await;
    assert_eq!(status, StatusCode::FORBIDDEN);

    // Doctor2 cannot see Doctor1's groups
    let (status, body) = app.get_with_auth("/api/v1/patient-groups", &doctor2_token).await;
    assert_eq!(status, StatusCode::OK);
    assert_eq!(body["data"].as_array().unwrap().len(), 0);
...
async fn test_patient_group_duplicate_name() {
    ...
    let (status, _) = app.post_with_auth("/api/v1/patient-groups", create_dto.clone(), &doctor_token).await;
    assert_eq!(status, StatusCode::OK);

    // Try to create group with same name
    let (status, body) = app.post_with_auth("/api/v1/patient-groups", create_dto, &doctor_token).await;
    assert_eq!(status, StatusCode::CONFLICT);
    assert_eq!(body["message"].as_str().unwrap(), "Group name already exists");
}
