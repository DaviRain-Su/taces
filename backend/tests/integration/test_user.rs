...
async fn test_get_user_by_id() {
    ...
    // Create test users
    let (patient_id, patient_account, patient_password) = create_test_user(&app.pool, "patient").await;
...
async fn test_batch_delete_users() {
    ...
    assert_eq!(status, StatusCode::OK);
    assert_eq!(body["success"], true);
    assert!(body["message"].as_str().unwrap().contains("2 users deleted"));
}
...
async fn test_export_users() {
    ...
    assert!(body["data"].as_str().unwrap().contains("Account,Name,Gender,Phone,Email,Role,Status,Created At"));
}