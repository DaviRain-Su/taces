...
async fn test_create_upload_url() {
    ...
    let (status, body) = app.post_with_auth("/api/v1/files/upload", create_dto, &token).await;
...
async fn test_get_file() {
    ...
    // Get file
    let (status, body) = app.get_with_auth(&format!("/api/v1/files/{}", file_id), &token).await;
...
async fn test_list_files() {
    ...
    // List files
    let (status, body) = app.get_with_auth("/api/v1/files?page=1&page_size=10", &token).await;
...
async fn test_delete_file() {
    ...
    // Delete file
    let (status, _) = app.delete_with_auth(&format!("/api/v1/files/{}", file_id), &token).await;
...
async fn test_authorization() {
    ...
    let (status, _) = app.get_with_auth(&format!("/api/v1/files/{}", file_id), &user2_token).await;
    
    assert_eq!(status, StatusCode::FORBIDDEN);
    
    // User2 tries to delete user1's file
    let (status, _) = app.delete_with_auth(&format!("/api/v1/files/{}", file_id), &user2_token).await;
...
async fn test_public_file_access() {
    ...
    let (status, body) = app.get_with_auth(&format!("/api/v1/files/{}", file_id), &user2_token).await;
