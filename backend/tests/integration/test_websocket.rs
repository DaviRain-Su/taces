use backend::services::websocket_service::{WebSocketManager, WsMessage};
use std::sync::Arc;
use uuid::Uuid;
...
async fn test_websocket_broadcast_to_role() {
    ...
    let mut rx_doctor1 = ws_manager
        .add_connection(doctor1, "doctor".to_string())
        .await;
    let mut rx_doctor2 = ws_manager
        .add_connection(doctor2, "doctor".to_string())
        .await;
    let mut rx_patient = ws_manager
        .add_connection(patient, "patient".to_string())
        .await;
...
async fn test_websocket_broadcast_to_all() {
    ...
    let mut rx1 = ws_manager
        .add_connection(user1, "patient".to_string())
        .await;
...
async fn test_websocket_notification_message() {
    ...
    let mut rx = ws_manager
        .add_connection(user_id, "patient".to_string())
        .await;
...
async fn test_websocket_live_stream_events() {
    ...
    ws_manager
        .broadcast_live_stream_started(stream_id, "Health Talk".to_string(), "Dr. Wang".to_string())
        .await;

    // Verify message received
    if let Ok(received_msg) = rx.try_recv() {
        match received_msg {
            WsMessage::LiveStreamStarted {
                stream_id: id,
                title,
                host_name,
            } => {
...
async fn test_websocket_video_call_request() {
    ...
    let _rx_doctor = ws_manager
        .add_connection(doctor_id, "doctor".to_string())
        .await;
    let mut rx_patient = ws_manager
        .add_connection(patient_id, "patient".to_string())
        .await;

    // Send video call request
    ws_manager
        .send_video_call_request(consultation_id, doctor_id, patient_id)
        .await;

    // Verify patient received request
    if let Ok(received_msg) = rx_patient.try_recv() {
        match received_msg {
            WsMessage::VideoCallRequest {
                consultation_id: id,
                from_user_id,
                to_user_id,
            } => {
