...
impl TestApp {
    ...
    pub async fn post_with_auth<T>(&mut self, path: &str, body: T, token: &str) -> (StatusCode, Value)
    where
        T: serde::Serialize,
    {
        let request = Request::builder()
    ...
    pub async fn put_with_auth<T>(&mut self, path: &str, body: T, token: &str) -> (StatusCode, Value)
    where
        T: serde::Serialize,
    {
        let request = Request::builder()
