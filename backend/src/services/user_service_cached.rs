...
pub async fn get_user_by_id_cached(
    ...
    let cache_key = CacheKeys::user(&id);
...
pub async fn get_user_by_email_cached(
    ...
    // Also cache by ID
    let id_cache_key = CacheKeys::user(&user.id);
...
pub async fn create_user_cached(
    ...
    let user = user_service::create_user(pool, dto).await?;
...
pub async fn update_user_cached(
    ...
    let user = user_service::update_user(pool, id, dto).await?;
    ...
    let email_cache_key = CacheKeys::user_email(&user.email);
...
pub async fn list_users_cached(
    ...
    if search.is_some() || role.is_some() {
        return user_service::list_users(pool, page, per_page, search, role).await;
    }
    ...
    // Cache miss, fetch from database
    let users = user_service::list_users(pool, page, per_page, None, None).await?;
