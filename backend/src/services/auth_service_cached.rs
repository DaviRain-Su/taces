...
pub async fn logout_cached(redis: &Option<RedisPool>, token: &str) -> Result<()> {
    // Invalidate session
...
pub async fn validate_token_cached(
    ...
    if let Some(session) = SessionService::get_session(redis, token).await {
        ...
        return user_service_cached::get_user_by_id_cached(pool, redis, session.user_id)
            .await
    ...
    let user = user_service_cached::get_user_by_id_cached(pool, redis, claims.sub)
        .await
        .map_err(|e| anyhow!("Failed to get user: {}", e))?;
