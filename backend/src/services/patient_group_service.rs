...
pub async fn get_group_by_id(pool: &DbPool, id: Uuid, doctor_id: Uuid) -> Result<PatientGroupWithMembers> {
    // First get the group
    let group_query = r#"
        SELECT 
            pg.id, 
...
pub async fn create_group(pool: &DbPool, doctor_id: Uuid, dto: CreatePatientGroupDto) -> Result<PatientGroup> {
...
pub async fn update_group(pool: &DbPool, id: Uuid, doctor_id: Uuid, dto: UpdatePatientGroupDto) -> Result<PatientGroup> {
    ...
    list_doctor_groups(pool, doctor_id).await?
        .into_iter()
        .find(|g| g.id == id)
...
pub async fn add_members(pool: &DbPool, group_id: Uuid, doctor_id: Uuid, patient_ids: Vec<Uuid>) -> Result<()> {
    // First verify ownership
    let check_query = "SELECT id FROM patient_groups WHERE id = ? AND doctor_id = ?";
    sqlx::query(check_query)
        .bind(group_id.to_string())
...
pub async fn remove_members(pool: &DbPool, group_id: Uuid, doctor_id: Uuid, patient_ids: Vec<Uuid>) -> Result<()> {
    // First verify ownership
    let check_query = "SELECT id FROM patient_groups WHERE id = ? AND doctor_id = ?";
    sqlx::query(check_query)
        .bind(group_id.to_string())
    ...
    // Remove members
    for patient_id in patient_ids {
        let delete_query = "DELETE FROM patient_group_members WHERE group_id = ? AND patient_id = ?";
...
pub async fn send_group_message(pool: &DbPool, group_id: Uuid, doctor_id: Uuid, _message: &str) -> Result<Vec<String>> {
...
fn parse_patient_group_from_row(row: &sqlx::mysql::MySqlRow) -> Result<PatientGroup> {
    ...
    Ok(PatientGroup {
        ...
        doctor_id: Uuid::parse_str(row.get("doctor_id")).map_err(|e| anyhow!("Failed to parse UUID: {}", e))?,
...
fn parse_group_member_from_row(row: &sqlx::mysql::MySqlRow) -> Result<PatientGroupMember> {
    ...
    Ok(PatientGroupMember {
        id: Uuid::parse_str(row.get("id")).map_err(|e| anyhow!("Failed to parse UUID: {}", e))?,
        group_id: Uuid::parse_str(row.get("group_id")).map_err(|e| anyhow!("Failed to parse UUID: {}", e))?,
        patient_id: Uuid::parse_str(row.get("patient_id")).map_err(|e| anyhow!("Failed to parse UUID: {}", e))?,
...
fn parse_group_with_members(row: &sqlx::mysql::MySqlRow, members: Vec<PatientGroupMember>) -> Result<PatientGroupWithMembers> {
    use sqlx::Row;
