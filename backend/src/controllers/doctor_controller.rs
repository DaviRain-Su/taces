...
pub async fn list_doctors(
    ...
    match doctor_service::list_doctors(&pool, page, per_page, query.department, query.search).await {
...
pub async fn get_doctor(
    ...
    match doctor_service::get_doctor_by_id(&pool, id).await {
...
pub async fn get_doctor_by_user_id(
    ...
    match doctor_service::get_doctor_by_user_id(&pool, user_id).await {
        Ok(doctor) => Ok(Json(ApiResponse::success("Doctor retrieved successfully", doctor))),
        Err(e) => Err((
            StatusCode::NOT_FOUND,
            Json(ApiResponse::error(&format!("Doctor not found: {}", e))),
...
pub async fn create_doctor(
    ...
    match doctor_service::create_doctor(&pool, dto).await {
...
pub async fn update_doctor(
    ...
    let doctor = match doctor_service::get_doctor_by_id(&pool, id).await {
        Ok(d) => d,
        Err(_) => return Err((
            StatusCode::NOT_FOUND,
    ...
    match doctor_service::update_doctor(&pool, id, dto).await {
...
pub async fn update_doctor_photos(
    ...
    match doctor_service::update_doctor_photos(&pool, id, photos).await {
